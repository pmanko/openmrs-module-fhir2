...
public class TaskTranslatorImpl implements TaskTranslator {
	...
	private void setFhirTaskFields(FhirTask openmrsTask, Task fhirTask) {
		...
		if (openmrsTask.getBasedOnReferences() != null) {
			fhirTask.setBasedOn(openmrsTask.getBasedOnReferences().stream().map(this::translateFromStringReference).collect(Collectors.toList()));
		}
		...
		if (openmrsTask.getOutputReferences() != null) {
			fhirTask.setOutput(openmrsTask.getOutputReferences().stream().map(this::translateOutputReferences).collect(Collectors.toList()));
		}
	}
	...
	private Reference translateFromStringReference(String stringRef) {
		...
		return new Reference().setReference(splitRef[0] + "/" + splitRef[1])
